...
void ControlSequence::start()
{
    ...
    while(ros::ok()){
        cylinder_pub.publish(marker_array);
        pose_stamped.pose = disk.getPose();
        pose_pub.publish(pose_stamped);
        pose_stamped.pose = work_point;
        work_pose_pub.publish(pose_stamped);
        ros::Duration(0.5).sleep();
    }
    sensor_msgs::PointCloud2ConstPtr scene_cloud_ptr =
